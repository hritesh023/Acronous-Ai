...
class MemorySystem:
    ...
    def get_stats(self):
        ...
        self.cursor.execute("SELECT COUNT(*) FROM knowledge WHERE timestamp > ?",
                           ((datetime.now() - timedelta(hours=24)).isoformat(),))
        recent_knowledge = self.cursor.fetchone()[0]
        self.cursor.execute("SELECT AVG(confidence) FROM knowledge")
        ...
        return {
            ...
            "recent_knowledge_24h": recent_knowledge,
            "avg_confidence": round(avg_confidence, 3) if avg_confidence else 0,
        }

    def get_fresh_knowledge(self, hours=24, limit=20):
        """Get knowledge learned in the last N hours (most recent first)."""
        cutoff = (datetime.now() - timedelta(hours=hours)).isoformat()
        self.cursor.execute(
            "SELECT key, value, source, confidence, timestamp FROM knowledge WHERE timestamp > ? ORDER BY timestamp DESC LIMIT ?",
            (cutoff, limit)
        )
        return [{"key": r[0], "value": r[1], "source": r[2], "confidence": r[3], "timestamp": r[4]}
                for r in self.cursor.fetchall()]

    def get_stale_knowledge(self, days=30, min_confidence=0.3, limit=50):
        """Get knowledge older than N days that might need refresh."""
        cutoff = (datetime.now() - timedelta(days=days)).isoformat()
        self.cursor.execute(
            "SELECT key, value, source, confidence, timestamp FROM knowledge WHERE timestamp < ? AND confidence >= ? ORDER BY confidence ASC LIMIT ?",
            (cutoff, min_confidence, limit)
        )
        return [{"key": r[0], "value": r[1], "source": r[2], "confidence": r[3], "timestamp": r[4]}
                for r in self.cursor.fetchall()]

    def verify_knowledge(self, key, new_source, confidence_boost=0.1):
        """Boost confidence when a fact is verified by a second source."""
        self.cursor.execute(
            "UPDATE knowledge SET confidence = MIN(1.0, confidence + ?) WHERE key = ?",
            (confidence_boost, key)
        )
        self.conn.commit()
